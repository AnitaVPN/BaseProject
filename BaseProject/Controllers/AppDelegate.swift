...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    var userModelClass = UserModelClass()
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        let userId = UserDefaults.standard.value(forKey: Constant.user_id) as? String
        if(userId == nil)
        {
            setupRootVC()
        }
...
//        setupRootVC()
    ...
    func setupRootVCWithTab(isEnableSideMenu:Bool)
    {
        ...
        let tab_4 = Notification.init(nibName: "Notification", bundle: nil)
        let tab_5 = Profile.init(nibName: "Profile", bundle: nil)
        
        
        //setup the tab bar elements with the icons, name and initial view controllers
        let vcData: [(UIViewController, UIImage)] = [
            (tab_1, UIImage(named: "share-1")!),
            (tab_2, UIImage(named: "search-1")!),
            (tab_3, UIImage(named: "plus-1")!),
            (tab_4, UIImage(named: "notification-1")!),
            (tab_5, UIImage(named: "user-1")!)
        ]
        
        let vcs = vcData.map { (vc, image) -> UINavigationController in
        ...
        tabController.tabBar.tintColor = UIColor.init(red: 0/255, green: 163/255, blue: 185/255, alpha: 1.0)
